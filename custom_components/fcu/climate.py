...
class FCUClimateEntity(ClimateEntity):
    ...
    def __init__(self, name, ip_address, hass):
        self._name = name
        ...
        self._token = None  # Initialize the token as None
        self._coordinator = DataUpdateCoordinator(
            hass,
            _LOGGER,
            name=f"{name}_status_update",
    ...
    async def async_added_to_hass(self):
        ...
        try:
            await self._update_token()  # Ensure the token is updated during setup
        except Exception as e:
            _LOGGER.error("Failed to update token during setup: %s", e)
    ...
    async def async_will_remove_from_hass(self):
        ...
        _LOGGER.info("Removing FCUClimateEntity from Home Assistant: %s", self._name)
        ...
        try:
            ...
            async with session.get(
                ...
                if response.status == 404:
                    _LOGGER.error("Status endpoint not found for %s. URL: %s", self._name, response.url)
                    self._available = False
                    return
    ...
    async def _update_token(self):
        ...
        try:
            ...
            _LOGGER.debug("Retrieved token: %s", self._token)
            if not self._token:
...
async def async_setup_entry(
    ...
    async_add_entities([FCUClimateEntity(name, ip_address, hass)])